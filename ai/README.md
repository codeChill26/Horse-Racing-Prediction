...
### Feature engineering: winrate của jockey
Đây là điểm cộng. Cách tính **chống leakage** (trong `build_winrate_table`):
1. **Chỉ tính winrate từ tập TRAIN**, rồi "tra bảng" áp sang test. Nài chưa từng thấy → gán winrate trung bình chung.
...
**Hệ số model học được** (in ra khi train) đều hợp trực giác: `RPR ↑`, `jockey_winrate ↑` → tăng khả năng thắng; `runners ↑`, `age ↑` → giảm. (Lưu ý `OR` ra hệ số âm do **tương quan mạnh với RPR** — multicollinearity, không phải lỗi.)
...
**Bước 5 — Thêm feature engineering** (winrate jockey) và xem chỉ số có tăng không. Nhớ chống leakage (tính winrate chỉ từ train).
